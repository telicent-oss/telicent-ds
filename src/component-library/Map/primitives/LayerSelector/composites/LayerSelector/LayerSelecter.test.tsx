...
describe("useLayerSelector", () => {
  let result: RenderHookResult<
    ReturnType<typeof useLayerSelector>,
    void
  >["result"];
  ...
  beforeEach(() => {
    ...
    result = renderHook(() =>
      useLayerSelector({ data: initialData, onChange: onChangeMock })
    ).result;
  });
...
describe("LayerSelector", () => {
  ...
  test("should not render if data has one or no item", () => {
    const { container } = render(
      <LayerSelector data={[mockData[0]]} onChange={mockOnChange} />
    );
  ...
  test("should render and handle dropdown interactions", () => {
    const result = render(
      <LayerSelector data={mockData} onChange={mockOnChange} />
    );
