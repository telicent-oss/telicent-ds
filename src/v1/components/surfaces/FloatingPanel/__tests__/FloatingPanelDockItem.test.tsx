...
const renderComponent = (count?: number) => {
  return renderWithUser(<FloatingPanelDockItem label="Toggle map" targetId="map" count={count} />, {
...
describe("Floating panel dock item component", () => {
  ...
  test.skip("The toggleMinimised function is called with the value of the targetId prop", async () => {
    ...
    await user.click(within(listitem).getByRole("button", { name: "Toggle map" }));
  ...
  test("renders total count when number is greater than 0", () => {
    ...
    expect(within(listitem).getByRole("button", { name: "Toggle map (2)" })).toBeVisible();
  });
});
