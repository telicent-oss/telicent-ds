...
const DropdownButtonMenuItem: React.FC<DropDownButtonMenuItemProps> = ({
  onClick,
  faIcon,
  id,
  icon,
...
const DropdownButton: React.FC<DropdownButtonProps> = ({ ariaLabel, id, menuItems }) => {
  ...
  const [selectedMenuItem, setSelectedMenuItem] = useState(
    menuItems.find((menuItem) => menuItem.default) ?? menuItems[0]
  );
  const theme = useExtendedTheme();
  ...
  return (
    <FlexBox>
      ...
      >
        ...
        {selectedMenuItem.faIcon && (
          <FontAwesomeIcon
            icon={selectedMenuItem.faIcon}
            fontSize="medium"
